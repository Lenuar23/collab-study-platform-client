...
public class MockApiClient implements ApiClient {
    ...
    public MockApiClient() {
        ...
        demo.setAbout("Learning together!");
        users.put(1L, demo);
        ...
        // Conversation Demo <-> Alice
        Conversation c = new Conversation(nextConvId(), 1L, 2L, LocalDateTime.now().minusMinutes(1));
        conversations.put(c.getId(), c);
        List<ChatMessage> list = new ArrayList<>();
        list.add(new ChatMessage(nextMsgId(), c.getId(), 2L, 1L, "Hi! How is studying going?", LocalDateTime.now().minusMinutes(5)));
        list.add(new ChatMessage(nextMsgId(), c.getId(), 1L, 2L, "Hey! Not bad, I will add new tasks soon.", LocalDateTime.now().minusMinutes(3)));
        list.add(new ChatMessage(nextMsgId(), c.getId(), 2L, 1L, "Nice, ping me when you're online.", LocalDateTime.now().minusMinutes(1)));
    ...
    @Override
    public User updateUserProfile(long id, String name, String about, String avatarUrl) {
    ...
    @Override
    public Group updateGroupProfile(long id, String name, String description, String avatarUrl) {
        Group g = getGroupById(id);
        if (name != null && !name.isBlank()) g.setName(name);
        if (description != null) g.setDescription(description);
        if (avatarUrl != null) g.setAvatarUrl(avatarUrl);
        return g;
    }
}
